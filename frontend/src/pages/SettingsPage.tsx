...
import { GetConfig, SaveConfig, GetDBPath, ChooseDirectory, GetBackupStatus, TriggerBackupNow, LoadTemplates, SaveTemplates, GetVersion, GetDocReminderSettings, SaveDocReminderSettings } from '../../wailsjs/go/main/App';
...
export default function SettingsPage({ onClose }: SettingsPageProps) {
    ...
    const [initialReminderEnabled, setInitialReminderEnabled] = useState(false);
    const [initialReminderInterval, setInitialReminderInterval] = useState(30);
    const [reminderSavedMsg, setReminderSavedMsg] = useState('');
    ...
    const isReminderDirty = reminderEnabled !== initialReminderEnabled || reminderInterval !== initialReminderInterval;

    const hasUnsavedChanges = isDirty || isReminderDirty;

    useEffect(() => {
        Promise.all([GetConfig(), GetDBPath(), GetBackupStatus(), LoadTemplates(), GetVersion(), GetDocReminderSettings()])
            .then(([c, p, bs, tpls, ver, rem]) => {
                setCfg(c);
                setInitialCfg(c);
                setDbPath(p);
                setBackupStatus(bs);
                ...
                setReminderInterval(rem.intervalMinutes);
                setInitialReminderEnabled(rem.enabled);
                setInitialReminderInterval(rem.intervalMinutes);
            })
    ...
    const handleSaveReminder = async () => {
        setReminderError('');
        try {
            await SaveDocReminderSettings(reminderEnabled, reminderInterval);
            setInitialReminderEnabled(reminderEnabled);
            setInitialReminderInterval(reminderInterval);
            setReminderSavedMsg('Saved');
            setTimeout(() => setReminderSavedMsg(''), 2000);
        } catch (err: unknown) {
    ...
    return (
        ...
        >
            <div className="relative bg-[var(--bg-primary)] w-full max-w-2xl mx-4 rounded-xl shadow-2xl max-h-[90vh] flex flex-col">
                {/* Header */}
                <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[var(--border-color)] shrink-0">
                ...
                {/* Scrollable content */}
                <div className="overflow-y-auto px-6 py-6 space-y-0">
                    ...
                    <section className="mb-8">
                        <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Documentation Reminder</h2>
                        <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 space-y-4">
                            <label className="flex items-center gap-3 cursor-pointer">
                                <input
                                    type="checkbox"
                                    ...
                                    className="rounded"
                                />
                                <span className="text-sm text-gray-200">Enable documentation reminders</span>
                            </label>

                            <div>
                                <label className="block text-xs text-gray-400 mb-1">Reminder interval (minutes)</label>
                                <input
                                    type="number"
                                    min={1}
                                    value={reminderInterval}
                                    onChange={(e) => setReminderInterval(Math.max(1, Number(e.target.value)))}
                                    className="w-32 px-3 py-1.5 bg-gray-700 border border-gray-600 rounded text-sm text-gray-200 focus:outline-none focus:border-blue-500"
                                />
                                {reminderInterval < 30 && (
                                    <p className="mt-1.5 text-xs text-amber-500">
                                        Intervals below 30 minutes may cause frequent partial reset notifications.
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="flex items-center gap-4 pt-1">
                                <button
                                    ...
                                    className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded transition-colors text-sm"
                                >
                                    Save
                                </button>
                                {isReminderDirty && !reminderSavedMsg && (
